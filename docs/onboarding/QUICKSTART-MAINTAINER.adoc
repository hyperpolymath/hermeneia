...
// Replace rsr-template-repo, hermeneia, {{DEPS}} with actuals
...
| `$PREFIX/share/hermeneia/`
| Data files, assets

| `$PREFIX/share/doc/hermeneia/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/hermeneia/config.toml`

Fallback: `$HOME/.config/hermeneia/config.toml`
...
Or via OPSM: `opsm update hermeneia`
...
[source,bash]
----
just install --prefix=/opt/hermeneia-instance1 --config=/etc/hermeneia/instance1.toml
just install --prefix=/opt/hermeneia-instance2 --config=/etc/hermeneia/instance2.toml
----
